...
function [x,phi,VSINR] = get_x_phi_ZF(Prms,Channel,S)
...
sc = norm(B1);
phi = get_initial_phi(Prms,B1/sc,C1/sc,B2/sc,C2/sc);
Phi2 = phi*phi.';
...
while iter <= Nmax && res > res_th
    ...
    for i = 1:1:M*L
        Dt(i,i) = real(Dt(i,i)) + 1e-5*real(Dt(1,1));
    end
    sc = norm(dt,2)^2;
    Dt = Dt/sc;
    dt = dt/sc;
    ...
    subject to
    Hkl*x + Gkl_tilde*phi == al.*gamma_ZF;
    al >= 1;
    abs(phi) <= 1;
    cvx_end
