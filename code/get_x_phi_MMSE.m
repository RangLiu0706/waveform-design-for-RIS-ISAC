...
Hkl = zeros(K*L,M*L);
gamma_ZF = zeros(K*L,1);
for ell = 1:1:L
    for k = 1:1:K
        Hkl((ell-1)*K+k,:) = kron(Iell(ell,:),Hu(k,:));
        gamma_ZF((ell-1)*K+k) = gamma(k)*S(k,ell);
    end
end
sc = abs(Hkl(1,1));
Hkl = Hkl/sc;
Gk = Gk/sc;
gamma_ZF = gamma_ZF/sc;
epsilon = epsilon/sc/sc;
...
while iter <= Nmax && res > res_th
    ...
    subject to
    square_abs(Hkl_tilde*x-al.*gamma_ZF) <= epsilon;
    al >= 1;
    abs(x) <= sqrt(P/M/L);
    cvx_end
    ...
    subject to
    square_abs(Hkl*x + Gkl_tilde*phi-al.*gamma_ZF) <= epsilon;
    al >= 1;
    abs(phi) <= 1;
    cvx_end
    %%%% update y
