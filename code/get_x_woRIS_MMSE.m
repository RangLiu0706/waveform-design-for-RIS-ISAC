% The proposed MMSE scheme without RIS.
...
function [x,VSINR] = get_x_woRIS_MMSE(Prms,Channel,S,epsilon)
...
while iter <= Nmax && res > res_th
    ...
    subject to
    square_abs(Hkl*x-al.*gamma_ZF) <= epsilon;
    al >= 1;
    abs(x) <= sqrt(P/M/L);
    cvx_end
