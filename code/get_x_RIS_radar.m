% The radar-only system with the aid of RIS.
...
function [x,phi,VSINR] = get_x_RIS_radar(Prms,Channel)

M = Prms.M; N = Prms.N; L = Prms.L; Q = Prms.Q; sigma2 = Prms.sigma2;
sigmar2 = Prms.sigmar2;  P = Prms.P; Nmax = Prms.Nmax;
res_th = Prms.res_th; ht = Channel.ht; hrt = Channel.hrt;
Hc = Channel.Hc; Hrc = Channel.Hrc; G = Channel.G;
...
Ht = zeros(Q+1,M);
for q = 1:1:Q
    Ht(q,:) = Hc(q,:) + Hrc(q,:)*diag(phi)*G;
end
Ht(end,:) = ht + hrt*diag(phi)*G;
Ht = Ht/norm(Ht);
