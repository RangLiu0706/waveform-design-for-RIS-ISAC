...
N_sim = 10000;
SINR_my_CI = zeros(1,length(d_m_range));
SINR_my_ZF = zeros(1,length(d_m_range));
SINR_my_MMSE = zeros(1,length(d_m_range));
SINR_radar_only = zeros(1,length(d_m_range));
SINR_wo_RIS_radar = zeros(1,length(d_m_range));
SINR_wo_RIS_CI = zeros(1,length(d_m_range));
SINR_wo_RIS_ZF = zeros(1,length(d_m_range));
SINR_wo_RIS_MMSE = zeros(1,length(d_m_range));
...
for sim = 1:1:N_sim
    ...
    SINR_wo_RIS_radar = SINR_wo_RIS_radar + VSINR_radar_wo_RIS(end)*ones(1,length(d_m_range));

    [x_wo_RIS,VSINR_wo_RIS] = get_x_woRIS_CI(Prms,Channel,S);
    SINR_wo_RIS_CI = SINR_wo_RIS_CI + VSINR_wo_RIS(end)*ones(1,length(d_m_range));

    [x_wo_RIS_MMSE,VSINR_wo_RIS_MMSE] = get_x_woRIS_MMSE(Prms,Channel,S,epsilon);
    SINR_wo_RIS_MMSE = SINR_wo_RIS_MMSE + VSINR_wo_RIS_MMSE(end)*ones(1,length(d_m_range));

    [x_wo_RIS_ZF,VSINR_wo_RIS_ZF] = get_x_woRIS_ZF(Prms,Channel,S);
    SINR_wo_RIS_ZF = SINR_wo_RIS_ZF + VSINR_wo_RIS_ZF(end)*ones(1,length(d_m_range));

    phi = exp(1i*2*pi*rand(N,1));
    for d_index = 1:1:length(d_m_range)
        drt = d_m_range(d_index)

        Channel.hrt = hrt*sqrt(10^(-3)*drt^(-alpha_rt));
        for q = 1:1:Q
            Channel.Hrc(q,:) = sqrt(10^(-3)*(drt+dcell*clutter(q,1))^(-alpha_rt))*Hrc(q,:);
        end

        [x_radar,phi_radar,VSINR_radar] = get_x_RIS_radar(Prms,Channel);
        SINR_radar_only(d_index) = SINR_radar_only(d_index) + VSINR_radar(end);

        [x_my,phi_my,VSINR_my] = get_x_phi_CI(Prms,Channel,S);
        SINR_my_CI(d_index) = SINR_my_CI(d_index) + VSINR_my(end);

        [x_ZF,phi_ZF,VSINR_ZF] = get_x_phi_ZF(Prms,Channel,S);
        SINR_my_ZF(d_index) = SINR_my_ZF(d_index) + VSINR_ZF(end);

        [x_MMSE,phi_MMSE,VSINR_MMSE] = get_x_phi_MMSE(Prms,Channel,S,epsilon);
        SINR_my_MMSE(d_index) = SINR_my_MMSE(d_index) + VSINR_MMSE(end);
...
figure
plot(d_m_range,SINR_my_CI,'-o','color',[0.8,0,0],'LineWidth',1.5)
hold on
plot(d_m_range,SINR_wo_RIS_CI,'--o','color',[0.8,0,0],'LineWidth',1.5)
plot(d_m_range,SINR_my_MMSE,'-s','color',[0,0,0.8],'LineWidth',1.5)
plot(d_m_range,SINR_wo_RIS_MMSE,'--s','color',[0,0,0.8],'LineWidth',1.5)
plot(d_m_range,SINR_my_ZF,'-^','color',[0.05,0.4,0.05],'LineWidth',1.5)
plot(d_m_range,SINR_wo_RIS_ZF,'--^','color',[0.05,0.4,0.05],'LineWidth',1.5)
plot(d_m_range,SINR_radar_only,'-x','color',[0,0,0],'LineWidth',1.5)
plot(d_m_range,SINR_wo_RIS_radar,'--x','color',[0,0,0],'LineWidth',1.5)
hold off
xlabel('RIS-target distance');
